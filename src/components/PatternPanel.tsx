  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-3 gap-4">
        <Card>
          <CardContent className="p-4 text-center">
            <div className="text-2xl font-bold text-green-500">{detectedPatterns.length}</div>
        ...
        <Card>
          <CardContent className="p-4 text-center">
            <div className="text-2xl font-bold">
              {Math.round(
                detectedPatterns.reduce((acc, p) => acc + p.confidence, 0) /
                  Math.max(detectedPatterns.length, 1)
              )}
              %
            </div>
            ...
            <div>
              ...
              <div className="flex flex-wrap gap-2">
                ...
                {sr.resistances.length === 0 && (
                  <span className="text-sm text-muted-foreground">No clear resistance levels</span>
                )}
              </div>
            </div>
            ...
            <div>
              ...
              <div className="flex flex-wrap gap-2">
                ...
                {sr.supports.length === 0 && (
                  <span className="text-sm text-muted-foreground">No clear support levels</span>
                )}
              </div>
            </div>
          </CardContent>
      ...
      <div>
        ...
        <div className="grid gap-2">
          {undetectedPatterns.map((pattern) => (
            <PatternCard key={pattern.name} pattern={pattern} />
          ))}
        </div>
      </div>
...
function PatternCard({
  ...
  pattern: FullAnalysis["patterns"][0];
}) {
  return (
        ...
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center gap-3">
            ...
            <Progress
              ...
              className={cn(
                "w-16 h-2",
                pattern.detected ? "bg-primary/20" : "bg-muted"
              )}
            />
          </div>
        </div>
