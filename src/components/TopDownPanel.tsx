...
export function TopDownPanel({ symbol }: TopDownPanelProps) {
  ...
  // Count signals
  const bullishCount = topDown.timeframes.filter(
    (t) => t.signal === "strong_buy" || t.signal === "buy"
  ).length;
  const bearishCount = topDown.timeframes.filter(
    (t) => t.signal === "strong_sell" || t.signal === "sell"
  ).length;
  ...
  return (
    <div className="space-y-6">
      ...
      <Card
        className={cn(
          "border-2",
          topDown.overallSignal === "strong_buy" || topDown.overallSignal === "buy"
            ? "border-green-500/30 bg-green-500/5"
            : topDown.overallSignal === "strong_sell" || topDown.overallSignal === "sell"
          ...
          <div className="grid grid-cols-3 gap-4 text-center">
            ...
            <div>
              <div className="flex items-center justify-center gap-1 text-yellow-500">
                ...
                <span className="text-lg font-bold">{neutralCount}</span>
              </div>
          ...
          {topDown.timeframes.map((tf) => {
            ...
            return (
              <Card
                ...
                className={cn(
                  "transition-all",
                  isHigherTF ? "border-l-4 border-l-primary" : "border-l-2 border-l-muted"
                )}
              >
                <CardContent className="p-4">
                  <div className="flex items-center justify-between">
                  ...
                  <div className="mt-2">
                    <Progress
                      value={Math.max(0, Math.min(100, 50 + tf.score))}
                      className={cn(
                        "h-1.5",
                        tf.score > 0
                          ? "bg-green-500/20"
                ...
                <div key={tf.timeframe} className="flex items-center">
                  ...
                  {i < topDown.timeframes.length - 1 && (
                    <div
                      className={cn(
                        "w-8 h-0.5 mx-1",
                        tf.signal === topDown.timeframes[i + 1].signal
                          ? "bg-primary"
                          : "bg-muted"
                      )}
                    />
