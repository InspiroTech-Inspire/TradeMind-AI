...
export function IndicatorPanel({ analysis, isLoading }: IndicatorPanelProps) {
  ...
  const trendIndicators = analysis.indicators.filter(
    (i) => ["SMA 20", "SMA 50", "EMA 12/26"].includes(i.name)
  );
  const momentumIndicators = analysis.indicators.filter(
    (i) => ["RSI 14", "MACD", "Stochastic"].includes(i.name)
  );
  const volatilityIndicators = analysis.indicators.filter(
    (i) => ["Bollinger Bands", "ATR"].includes(i.name)
  );
...
function IndicatorCard({
  ...
  indicator: FullAnalysis["indicators"][0];
}) {
  const valuePercent =
    indicator.name === "RSI 14"
      ? Math.min(100, Math.max(0, indicator.value))
      : indicator.name === "Stochastic"
        ? Math.min(100, Math.max(0, indicator.value))
        : indicator.name === "MACD"
          ? 50 + indicator.value * 10
          : 50;
    ...
    <Card className="hover:bg-muted/30 transition-colors">
      <CardContent className="p-4">
        ...
        {(indicator.name === "RSI 14" ||
          ...
          <div className="mt-2">
            <Progress
              value={Math.min(100, Math.max(0, valuePercent))}
              className="h-1.5"
            />
          </div>
        )}
