...
export function obv(ohlc: OHLCV[]): number[] {
  const result: number[] = [];
  let cumulative = 0;
  for (let i = 0; i < ohlc.length; i++) {
    if (i === 0) {
...
export function vwap(ohlc: OHLCV[]): number[] {
  ...
  for (let i = 0; i < ohlc.length; i++) {
    ...
    cumulativeTPV += tp * ohlc[i].volume;
    cumulativeVol += ohlc[i].volume;
    if (cumulativeVol === 0) {
      result.push(tp);
    } else {
...
export function findSupportResistance(
  ...
  // Sort by number of touches (strength) and return top levels
