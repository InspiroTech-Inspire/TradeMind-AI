...
// Analysis results (cached scan results)
export const analysisResults = mysqlTable("analysisResults", {
  id: serial("id").primaryKey(),
  symbol: varchar("symbol", { length: 20 }).notNull(),
  timeframe: varchar("timeframe", { length: 10 }).notNull(), // 1d, 4h, 1h, 30m, 15m, 5m
  analysisType: mysqlEnum("analysisType", ["indicator", "pattern", "topdown", "full"]).notNull(),
  // Overall scores
  trendScore: decimal("trendScore", { precision: 5, scale: 2 }), // -100 to 100
...
// Scan history (runs of the scanner)
export const scanHistory = mysqlTable("scanHistory", {
  id: serial("id").primaryKey(),
  userId: bigint("userId", { mode: "number", unsigned: true }).references(() => users.id),
  scanType: mysqlEnum("scanType", ["single", "watchlist", "market"]).notNull(),
  symbols: json("symbols"), // array of symbols scanned
  timeframes: json("timeframes"), // array of timeframes
  filters: json("filters"), // scan filters used
  resultsCount: int("resultsCount"),
  createdAt: timestamp("createdAt").defaultNow().notNull(),
});

// User alerts for triggered signals
